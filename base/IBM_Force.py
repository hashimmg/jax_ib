...
def IBM_Multiple_NEW(field, Xi, particles,discrete_fn,surface_fn,dt):
    ...
    return cfd.grids.GridArray(force,field.offset,field.grid)
...
def calc_IBM_force_NEW_MULTIPLE(all_variables,discrete_fn,surface_fn,dt):
    ...
    return tuple(cfd.grids.GridVariable(ibm_forcing(field,Xi),field.bc) for field,Xi in zip(velocity,axis))
